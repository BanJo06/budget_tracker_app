...
//       // Sort transactions by date descending for better grouping
...
//   // 2. Use useFocusEffect with a synchronous wrapper function (THE FIX)
//   useFocusEffect(
//     // This synchronous function is the required 'EffectCallback'
//     useCallback(() => {
//       // 🚨 We call the async function here immediately.
//       // The wrapper function returns nothing (void), satisfying TypeScript.
//       loadTransactions();

//       // Optional cleanup function can be returned here, but not needed for data fetch
//       return () => {};
//     }, [loadTransactions]) // Dependency array ensures the inner callback updates if loadTransactions changes
//   );
...
export default function Records() {
  ...
  // NEW: Add state to track refreshing status
  ...
  // NEW: Create the onRefresh handler
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    // Await the data loading function
  ...
  // ... (Modal rendering code remains exactly the same, omitted for brevity) ...
  const renderModalContent = () => {
    if (!selectedTransaction) {
      return null;
    }
  ...
  return (
    <View className="flex-1 px-8 bg-bgPrimary-light dark:bg-bgPrimary-dark">
      ...
      {/* If empty, we still want to allow refresh */}
      {transactions.length === 0 ? (
        <SectionList
          ...
          // FIX: Use refreshControl component instead of direct props
          refreshControl={
            <RefreshControl
              refreshing={refreshing}
              onRefresh={onRefresh}
              tintColor={colorScheme === "dark" ? "white" : "black"} // iOS
              colors={["#8938E9"]} // Android
            />
          }
          renderItem={() => null}
          ListEmptyComponent={
        ...
        <SectionList
          sections={sections}
          keyExtractor={(item) => item.id.toString()}
          // FIX: Use refreshControl component here as well
          refreshControl={
            <RefreshControl
              refreshing={refreshing}
              onRefresh={onRefresh}
          ...
          renderItem={({ item }) => {
            // ... (Your existing renderItem logic remains unchanged) ...
            // Copy your existing renderItem code here
            let IconComponent;
            let iconBgColorClass;
            let mainText;
            let amountText;
